...
fn main() {
    ...
    let requiredforupdate = 30000000;
    let usedspace = pwdata[0];
    let freespace = filesystem - usedspace;
    let needtofree = requiredforupdate - freespace;
    println!("need to free: {}",needtofree);
    let mut pwd: Vec<String> =  vec![];
    let mut pwdata: Vec<i32> = vec![];
    let mut lsmode = false;
    let mut suma = 0;
    let mut kandydat = 99999999; 
    //tak skopiowałem ten kod drugi raz - aż tak widać?
    for line in file_contents.lines(){
        if lsmode{
            if line.contains("$"){
                lsmode = false; 
        ...
        if lsmode == false{
            ...
            if line.contains("$ cd"){
                ...
                if lin == ".."{
                    if pwdata[(pwdata.len()-1)] >= needtofree{
                        suma+=pwdata[(pwdata.len()-1)];
                        if pwdata[(pwdata.len()-1)] < kandydat{
                            kandydat = pwdata[(pwdata.len()-1)];
                        }
                    }
                    pwd.remove((pwd.len()-1) as usize);
                    pwdata.remove((pwdata.len()-1) as usize);
